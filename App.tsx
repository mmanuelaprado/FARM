...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAdvice = async () => {
      try {
        ...
        setAdvice("O clima está ótimo para plantar!");
      }
    };
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-gradient-to-b from-sky-400 to-sky-200 overflow-hidden text-slate-800 touch-none">
      {floatingTexts.map(t => (
      ...
      <div className="z-10 px-3 pt-4 flex justify-between items-center shrink-0">
        <div className="flex flex-col gap-1">
          ...
          <div className="bg-white/90 backdrop-blur px-2 py-0.5 rounded-lg border border-blue-400 w-24">
            ...
            <span className="text-[8px] font-bold text-blue-600 block text-center uppercase mt-0.5">Nível {gameState.level}</span>
          </div>
        </div>

        <div className="flex items-center gap-2">
          <div className="flex bg-white/40 backdrop-blur p-0.5 rounded-xl border border-white/50">
             <button onClick={() => setActiveTab('crops')} className={`px-2 py-1.5 rounded-lg text-[9px] font-game ${activeTab === 'crops' ? 'bg-green-500 text-white shadow' : 'text-green-900'}`}>HORTA</button>
             <button onClick={() => setActiveTab('ranch')} className={`px-2 py-1.5 rounded-lg text-[9px] font-game ${activeTab === 'ranch' ? 'bg-orange-500 text-white shadow' : 'text-orange-900'}`}>RANCHO</button>
          </div>
          <button onClick={() => setIsStoreOpen(true)} className="bg-amber-500 p-2 rounded-xl shadow-lg active:scale-95 transition-all text-xl">🏪</button>
        </div>
      </div>

      {/* Advice Area */}
      <div className="z-10 px-3 mt-3 shrink-0">
        <div className="bg-white/95 rounded-2xl p-2 shadow-md border-b-2 border-green-500 flex items-center gap-2">
          <span className="text-xl shrink-0">👴</span>
          <p className="text-[10px] font-medium text-slate-700 italic leading-tight line-clamp-2">"{advice}"</p>
        </div>
      </div>

      {/* Grid Centralizado e Responsivo */}
      <div className="z-10 flex-1 overflow-y-auto px-3 py-4 scrollbar-hide pb-40">
        {activeTab === 'crops' ? (
          <div className="grid grid-cols-3 gap-2 max-w-sm mx-auto">
            ...
            {plots.length < MAX_PLOT_COUNT && (
              <button 
                ...
                className="aspect-square rounded-xl border-2 border-dashed border-white/50 bg-white/10 flex flex-col items-center justify-center active:scale-95"
              >
                <span className="text-lg opacity-50">➕</span>
                <span className="text-[8px] font-game opacity-50">${PLOT_UNLOCK_COST}</span>
              </button>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3 max-w-sm mx-auto">
            {gameState.animals.map(animal => {
              ...
              return (
                <div key={animal.id} onClick={(e) => collectAnimal(animal.id, e)} className="bg-white/90 p-3 rounded-2xl shadow-md border-2 border-orange-200 flex flex-col items-center relative active:scale-95">
                  <span className={`text-4xl mb-1 ${progress >= 100 ? 'animate-bounce' : ''}`}>{data.icon}</span>
                  <div className="w-full h-1.5 bg-orange-100 rounded-full overflow-hidden">
                     <div className="h-full bg-orange-500" style={{width: `${progress}%`}} />
                  </div>
                  {progress >= 100 && (
                    <div className="absolute -top-1 -right-1 bg-yellow-400 w-6 h-6 rounded-full border border-white flex items-center justify-center text-xs animate-pulse">
                      {data.produceIcon}
                    </div>
                  )}
                </div>
            ...
            <button onClick={() => { setStoreTab('animals'); setIsStoreOpen(true); }} className="aspect-square bg-white/20 border-2 border-dashed border-white/50 rounded-2xl flex items-center justify-center opacity-60">
      ...
      {/* Toolbar Inferior Mobile */}
      {activeTab === 'crops' && (
        <div className="absolute bottom-0 left-0 right-0 z-20 bg-amber-50/95 backdrop-blur-md p-3 pb-8 border-t border-amber-200 rounded-t-3xl shadow-2xl">
          <div className="flex justify-center gap-8 mb-4">
            <button onClick={() => setSelectedTool(ToolType.SEED)} className={`p-3 rounded-xl transition-all ${selectedTool === ToolType.SEED ? 'bg-amber-400 scale-110 shadow-lg' : 'bg-amber-100'}`}>🌱</button>
            <button onClick={() => setSelectedTool(ToolType.WATER)} className={`p-3 rounded-xl transition-all ${selectedTool === ToolType.WATER ? 'bg-blue-400 scale-110 shadow-lg' : 'bg-blue-100'}`}>💧</button>
          </div>
          <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide px-1">
            {(Object.keys(CROPS) as CropType[]).map(type => (
              <button 
                ...
                className={`min-w-[65px] p-1.5 rounded-xl border-2 transition-all relative shrink-0 ${selectedSeed === type ? 'bg-white border-amber-500 shadow-md' : 'bg-transparent border-transparent'}`}
              >
                <div className="absolute -top-1.5 -left-1 bg-amber-600 text-white text-[7px] px-1 py-0.5 rounded-full font-game">x{gameState.seedInventory[type] || 0}</div>
                <span className="text-xl block">{CROPS[type].icon}</span>
                <span className="text-[8px] font-bold uppercase truncate block">{CROPS[type].name}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {isStoreOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4">
          <div className="bg-white w-full max-w-md rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in slide-in-from-bottom duration-300">
            <div className="bg-amber-500 p-4 shrink-0 flex justify-between items-center text-white">
              <h2 className="font-game text-lg">MERCADO</h2>
              <button onClick={() => setIsStoreOpen(false)} className="text-2xl font-bold">&times;</button>
            </div>
            
            <div className="flex bg-amber-50 p-1 shrink-0">
              {['buy', 'animals', 'sell'].map((t) => (
                <button 
                  key={t}
                  onClick={() => setStoreTab(t as any)} 
                  className={`flex-1 py-2 text-[10px] font-game rounded-lg transition-all ${storeTab === t ? 'bg-amber-500 text-white shadow' : 'text-amber-800 opacity-60'}`}
                >
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {storeTab === 'buy' && (Object.keys(CROPS) as CropType[]).map(type => (
                <div key={type} className="flex items-center justify-between p-2 bg-amber-50 rounded-xl border border-amber-100">
                  <div className="flex items-center gap-2">
                    <span className="text-xl">{CROPS[type].icon}</span>
                    <span className="font-bold text-[10px] uppercase">{CROPS[type].name} <span className="text-amber-600 block text-[8px] font-game">${CROPS[type].cost}</span></span>
                  </div>
                  <button onClick={() => { if(gameState.coins>=CROPS[type].cost) setGameState(prev=>({...prev, coins: prev.coins-CROPS[type].cost, seedInventory: {...prev.seedInventory, [type]: (prev.seedInventory[type] || 0)+1}})) }} className="bg-green-500 text-white px-3 py-1 rounded-lg text-[9px] font-game">COMPRAR</button>
                </div>
              ))}
              ...
              {storeTab === 'sell' && Object.entries(gameState.inventory).map(([name, qty]) => {
                ...
                return (
                  <div key={name} className="flex items-center justify-between p-2 bg-green-50 rounded-xl border border-green-100">
                    <span className="text-xl">{icon} <span className="text-[10px] font-bold ml-2">{name} x{qty}</span></span>
                    <button onClick={() => setGameState(prev=>({...prev, coins: prev.coins+val, inventory: {...prev.inventory, [name]: (Number(prev.inventory[name]))-1}}))} className="bg-green-600 text-white px-3 py-1 rounded-lg text-[9px] font-game">VENDER (${val})</button>
                  </div>
                );
              })}
            </div>
